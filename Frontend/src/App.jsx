    ...
    <div className="card bg-white shadow-xl border border-slate-100">
      <div className="card-body p-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Live Menu</span>
        </div>
        ...
        {meal === "" ? (
          ...
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-700">
            <h2 className="card-title text-3xl font-black text-slate-800 mb-2">{menu}</h2>
            <p className="text-lg text-slate-600 leading-relaxed">{meal}</p>
          </div>
        )}
      </div>
    </div>
    ...
    return (
      <div className="card bg-white shadow-xl border border-slate-100 p-8">
        <div className="mb-6">
          ...
          <p className="text-slate-500">Want this menu delivered straight to your notifications?</p>
        </div>

        <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4">Pick your preferred meals</p>
        
        <div className="grid grid-cols-2 gap-4 mb-4">
          <button 
            className={`btn btn-md transition-all duration-300 ${breakfast ? "btn-neutral ring-4 ring-slate-200" : "btn-outline"}`} 
            onClick={() => setbreakfast(prev => !prev)}
          >
            🍳 Breakfast
          </button>
          <button 
            className={`btn btn-md transition-all duration-300 ${lunch ? "btn-neutral ring-4 ring-slate-200" : "btn-outline"}`} 
            onClick={() => setlunch(prev => !prev)}
          >
            🍲 Lunch
          </button>
          <button 
            className={`btn btn-md transition-all duration-300 ${snack ? "btn-neutral ring-4 ring-slate-200" : "btn-outline"}`} 
            onClick={() => setsnack(prev => !prev)}
          >
            ☕ Snacks
          </button>
          <button 
            className={`btn btn-md transition-all duration-300 ${dinner ? "btn-neutral ring-4 ring-slate-200" : "btn-outline"}`} 
        ...
        {status === "12" && (
         ...
         {status === "403" && (
          <div className="flex justify-center my-4">
            <span className="text-sm font-bold">Please allow notifications to stay updated!</span>
          </div>
        )}
          {status === "400" && (
          <div className="flex justify-center my-4">
            <span className="text-sm font-bold">Please Use Another Browser!</span>
          </div>
        )}
         {status === "200" && (
        ...
        <div>
          ...
          <p className="text-green-700 opacity-80">Congrats, you are officially notified! 😊</p>
        </div>
      </div>
        )}
        {status === "500" && (
          <div className="flex justify-center my-4">
            <span className="text-sm font-bold">Server Error,Please Try again</span>
          </div>
        )}

        {error && (
          <div className="alert alert-error shadow-sm py-2 mb-4">
            <span className="text-sm font-bold">Please select at least one meal!</span>
          </div>
        )}
